...
def search_venues():
    ...
    response = {}
    time = datetime.now()
    venue_count = Venue.query.filter(func.lower(Venue.name).like(
        '%'+func.lower(search_term)+'%')).count()
    for venue in Venue.query.filter(func.lower(Venue.name).like('%'+func.lower(search_term)+'%')).all():
        num_upcoming_shows = Show.query.filter(
            Show.venue_id == venue.id).filter(Show.start_time > time).count()
        venues = {
            "id": venue.id,
            "name": venue.name,
...
def search_artists():
    ...
    response = {}
    time = datetime.now()
    artist_count = Artist.query.filter(func.lower(
        Artist.name).like('%'+func.lower(search_term)+'%')).count()
    for artist in Artist.query.filter(func.lower(Artist.name).like('%'+func.lower(search_term)+'%')).all():
        num_upcoming_shows = Show.query.filter(
            Show.artist_id == artist.id).filter(Show.start_time > time).count()
        artists = {
            "id": artist.id,
            "name": artist.name,
            "num_upcoming_shows": num_upcoming_shows
